...
exports.parseSignalMessage = parseSignalMessage;
...
function isGarbargeMessage(text) {
...
function extractTakeProfits(text) {
    ...
    if (tp1Match)
        tps.push(parseFloat(tp1Match[1]));
    const tp2Match = text.match(/tp2[:\s]*([\d.]+)/i);
    if (tp2Match)
        tps.push(parseFloat(tp2Match[1]));
    const tp3Match = text.match(/tp3[:\s]*([\d.]+)/i);
    if (tp3Match)
        tps.push(parseFloat(tp3Match[1]));
...
function parseSignalMessage(text, sourceChatId) {
