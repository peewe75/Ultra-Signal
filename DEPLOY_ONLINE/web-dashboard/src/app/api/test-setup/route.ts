import { NextResponse } from "next/server";
import { createClerkClient } from "@clerk/nextjs/server";
import { getDb } from "@/lib/firebase";
import { makeKey, LicensePayload } from "@/lib/server/license_core";

export async function GET(req: Request) {
    try {
        const clerk = createClerkClient({ secretKey: process.env.CLERK_SECRET_KEY });
        const db = getDb();
        if (!db) return new NextResponse("No DB", { status: 500 });

        // 1. Create ADMIN USER
        let adminUser;
        try {
            adminUser = await clerk.users.createUser({
                emailAddress: ["admin_test@softibridge.com"],
                password: "TestSoftiBridge123!@",
                skipPasswordChecks: true
            });
        } catch (e: any) {
            if (e.errors && e.errors[0]?.code === 'form_identifier_exists') {
                const users = await clerk.users.getUserList({ emailAddress: ["admin_test@softibridge.com"] });
                adminUser = users.data[0];
            } else {
                throw e;
            }
        }

        if (adminUser) {
            await db.collection("users").doc(adminUser.id).set({
                telegram_id: 111111111,
                role: "admin",
                updated_at: new Date().toISOString()
            }, { merge: true });
        }

        // 2. Create CLIENT USER
        let clientUser;
        try {
            clientUser = await clerk.users.createUser({
                emailAddress: ["client_test@softibridge.com"],
                password: "TestSoftiBridge123!@",
                skipPasswordChecks: true
            });
        } catch (e: any) {
            if (e.errors && e.errors[0]?.code === 'form_identifier_exists') {
                const users = await clerk.users.getUserList({ emailAddress: ["client_test@softibridge.com"] });
                clientUser = users.data[0];
            } else {
                throw e;
            }
        }

        let clientLicenseKey = "";
        if (clientUser) {
            const clientTelegram = 999999999;
            await db.collection("users").doc(clientUser.id).set({
                telegram_id: clientTelegram,
                role: "client",
                updated_at: new Date().toISOString()
            }, { merge: true });

            // Generate PRO license
            const planName = "PRO";
            const nowTs = Math.floor(Date.now() / 1000);
            const expiryTs = nowTs + (30 * 86400);

            const payload: LicensePayload = {
                v: 2,
                product: "SOFTIBRIDGE_LITEB",
                telegram_id: clientTelegram,
                plan: planName,
                groups_limit: 3,
                accounts_limit: 3,
                allowed_accounts: [],
                iat: nowTs,
                exp: expiryTs
            };

            clientLicenseKey = makeKey(payload);

            // Check if license already exists
            const existingLicenses = await db.collection("users").doc(clientUser.id).collection("licenses").get();
            if (existingLicenses.empty) {
                await db.collection("users").doc(clientUser.id).collection("licenses").add({
                    license_key: clientLicenseKey,
                    plan: planName,
                    status: "ACTIVE",
                    expires_at: new Date(expiryTs * 1000).toISOString(),
                    created_at: new Date().toISOString(),
                    stripe_subscription_id: "mock_sub_test"
                });
            }
        }

        return NextResponse.json({
            success: true,
            message: "Test users created successfully.",
            users: {
                admin: {
                    email: "admin_test@softibridge.com",
                    password: "TestSoftiBridge123!@",
                    telegram_id: 111111111,
                },
                client: {
                    email: "client_test@softibridge.com",
                    password: "TestSoftiBridge123!@",
                    telegram_id: 999999999,
                    pro_license: clientLicenseKey
                }
            }
        });

    } catch (error: any) {
        console.error("Test setup error", error);
        if (error.errors) {
            console.error("Clerk errors:", JSON.stringify(error.errors, null, 2));
        }
        return NextResponse.json({ error: error.message }, { status: 500 });
    }
}
