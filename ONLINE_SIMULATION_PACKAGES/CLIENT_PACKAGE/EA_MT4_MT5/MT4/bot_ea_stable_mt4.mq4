// STABLE_BUILD: 3.71b-STABLE | bot_ea_stable
#property strict
// ================================
...
// Reads token from Common Files and blocks EA if missing/expired/mt not allowed/install mismatch
// ================================
...
string SB_ReadAllCommon(string path){
...
string SB_JsonGetStr(string json, string key){
   string k = "\""+key+"\"";
...
int SB_JsonGetInt(string json, string key){
   string k = "\""+key+"\"";
   int p = StringFind(json, k);
   if(p<0) return 0;
   p = StringFind(json, ":", p);
   ...
   return (int)StrToInteger(StringSubstr(json,p,e-p));
}

bool SB_JsonGetBool(string json, string key, bool defval){
   string k = "\""+key+"\"";
   int p = StringFind(json, k);
...
input int    SL_ExtraPips    = 20;    // default extra SL (pips) applied to EVERY format
...
input bool   AutoBE         = true;   // auto move SL to BE when profit >= BE_MinPips
input int    BE_MinPips     = 10;     // BE trigger in PIPS (digit-safe)
input int    BE_MinPoints   = 100;    // legacy (unused if BE_MinPips is set)
...
void WriteResult(string id, string status, string msg)
{
   ...
   if(h == INVALID_HANDLE)
   {
      ...
      Print("WriteResult failed err=", err, " file=", fname,
            " | NOTE: ensure Common/Files/softibridge/outbox exists.");
      return;
   }
...
if(g_MaxSpreadPoints > 0 && spreadPtsNow > g_MaxSpreadPoints)
   {
	      // Convert points -> pips using pip size (auto-handles 2/3/5 digit symbols)
      ...
      out_msg = "Blocked by MaxSpread: spread=" + DoubleToString(nowPips,1) + " pips (" + IntegerToString(spreadPtsNow) + " pts) > max=" + DoubleToString(cfgPips,1) + " pips (" + IntegerToString(g_MaxSpreadPoints) + " pts)";
      return false;
   }
...
bool useMarket = (distPips <= effThreshold);
// MARKET
   if(useMarket)
   {
...
void ProcessQueue()
{
   ...
   if(wait_id != "" && TimeCurrent() < wait_next_retry)
      return;

   // Open queue: try COMMON first (canonical), then LOCAL (older installs)
   bool q_common = true;
   string q_path = QUEUE_FILE;

   // Touch queue file once (creates empty file if folder exists)
   if(!FileIsExist(QUEUE_FILE, true))
   {
      int hw = FileOpen(QUEUE_FILE, FILE_WRITE|FILE_TXT|FILE_COMMON);
   ...
   if(h == INVALID_HANDLE)
   {
      // Fallback to LOCAL
      q_common = false;
      q_path = QUEUE_FILE_LOCAL;
      if(!FileIsExist(QUEUE_FILE_LOCAL, false))
      {
         int hw2 = FileOpen(QUEUE_FILE_LOCAL, FILE_WRITE|FILE_TXT);
         if(hw2 != INVALID_HANDLE) FileClose(hw2);
      }
   ...
   if(h == INVALID_HANDLE)
   {
      ...
      if((int)now - last_queue_warn_ts >= 15 || err != last_queue_warn_err)
      {
         Print("Queue open failed err=", err,
               " path(COMMON)=", QUEUE_FILE,
               " path(LOCAL)=", QUEUE_FILE_LOCAL,
               " (run create_folders_common_files.bat / bot must create cmd_queue.txt)");
         last_queue_warn_ts  = (int)now;
         last_queue_warn_err = err;
      }
      return;
   }

   // Process only ONE new command per timer tick (safer)
   while(!FileIsEnding(h))
   {
      ...
      // Skip already-processed IDs (persisted)
...
int OnInit()
{
   ...
   if(StringLen(token_json) <= 10){
      Print("E100 TOKEN MISSING: ", SB_TOKEN_FILE);
      return(INIT_FAILED);
   }

   // token contains {"payload":{...}}
   string payload_str = token_json;

   string status = SB_JsonGetStr(payload_str, "status");
   if(status == "SUSPENDED" || status == "REVOKED"){
      Print("E150 LICENSE BLOCKED: status=", status);
      return(INIT_FAILED);
   }

   int exp = SB_JsonGetInt(payload_str, "exp");
   int grace_period = 172800; // 48 hours in seconds
   
   if(exp > 0 && TimeCurrent() > exp + grace_period){
      Print("E120 TOKEN EXPIRED exp=", exp, " + grace period exceeded.");
      return(INIT_FAILED);
   }
   
   if(exp > 0 && TimeCurrent() > exp){
      Print("W120 GRACE PERIOD ACTIVE. Expiry was: ", exp);
   ...
   bool mt4_allowed = SB_JsonGetBool(payload_str, "mt4", true);
   if(!mt4_allowed){
      Print("E140 MT4 NOT ALLOWED");
      return(INIT_FAILED);
   }

   string token_install = SB_JsonGetStr(payload_str, "install_id");
   string local_install = StringTrimRight(StringTrimLeft(SB_ReadAllCommon(SB_INSTALL_FILE)));
   if(token_install != "" && local_install != "" && token_install != local_install){
      Print("E130 INSTALL_ID MISMATCH token=", token_install, " local=", local_install);
      return(INIT_FAILED);
   }
   // --------------------------------------
...
if(g_MaxSpreadPoints <= 0 && g_MaxSpreadPips > 0.0)
   ...
   LoadLastId();
   Print("SoftiBridge EA V_3.0.3 (LITE) avviato. Queue(COMMON)=", QUEUE_FILE,
...
bool IsBridgeOrder()
{
   // Accept orders created by this EA (MAGIC) or tagged by comment
   string c = OrderComment();
   if(OrderMagicNumber() == MAGIC) return true;
   if(StringFind(c, "SoftiBridge") >= 0) return true;
   return false;
}
...
bool SB_OrderClose(int ticket, double lots, double price)
{
   ...
   {
      int e = GetLastError();
      Print("[SB|bot_ea_stable] OrderClose failed. ticket=", ticket, " err=", e);
      ResetLastError();
   }
   return ok;
}
...
bool SB_OrderModify(int ticket, double openPrice, double sl, double tp)
{
   ...
   {
      int e = GetLastError();
      Print("[SB|bot_ea_stable] OrderModify failed. ticket=", ticket, " err=", e, " sl=", DoubleToString(sl,Digits), " tp=", DoubleToString(tp,Digits));
      ResetLastError();
   }
   return ok;
}
...
void CloseFiltered(int type)
{
   ...
   for(int i=OrdersTotal()-1;i>=0;i--)
   {
      ...
      else
      {
         ...
         {
            int e = GetLastError();
            Print("[SB|bot_ea_stable] OrderDelete failed. ticket=", OrderTicket(), " err=", e);
            ResetLastError();
         }
      }
   }

   Print("[SB|bot_ea_stable][UI|bot_ea_stable] CloseFiltered type=", type, " found=", foundCnt, " closed=", closedCnt, " deleted=", deletedCnt);
...
void MoveToBE()
{
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      // Profit in PIPS (digit-safe)
      double profitPips = (ot==OP_BUY) ? ((Bid - op)/pip) : ((op - Ask)/pip);
      if(profitPips < BE_MinPips) continue;

      // Target BE price with small offset (points)
      double be = op + (ot==OP_BUY ? (BE_OffsetPoints*pt) : -(BE_OffsetPoints*pt));

      // If already at/through BE, skip
      if(ot==OP_BUY)
      {
         if(sl >= be - (0.5*pt)) continue;
         // ensure stops respect stopLevel: SL must be <= Bid - stopLevel
         double maxSL = Bid - stopLevelPrice;
         if(be > maxSL) be = maxSL;
      }
      else // OP_SELL
      {
         if(sl != 0.0 && sl <= be + (0.5*pt)) continue;
         // ensure stops respect stopLevel: SL must be >= Ask + stopLevel
                  ...
                  const string &sparam)
{
   ...
   string n = sparam;
   Print("[UI|bot_ea_stable] click: ", n);
