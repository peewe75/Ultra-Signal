...
export async function POST(req: Request) {
  try {
    ...
    const expiryTs = nowTs + (14 * 86400); // 14 giorni di prova gratuita

    // Generiamo una chiave più leggibile per Telegram: BCS-XXXX-XXXX
    const randomPart = () => Math.random().toString(36).substring(2, 6).toUpperCase();
    const licenseKey = `BCS-${randomPart()}-${randomPart()}-${randomPart()}`;

    // 1. Salva nella subcollection dell'utente (per storico e dashboard)
    ...
    // 2. Salva nella collection globale per il bot Telegram
    await db.collection("licenses").add({
      userId: userId,
      licenseKey: licenseKey, // Campo usato dal listener.ts
      status: "ACTIVE",
      plan: planName,
      expiresAt: new Date(expiryTs * 1000), // Come timestamp
      createdAt: new Date()
    });
